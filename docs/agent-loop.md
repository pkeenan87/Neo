...
**Every Anthropic call is a stream under the hood.** The Anthropic SDK enforces a hard 10-minute wall-clock ceiling on non-streaming `messages.create()` calls. Skills routinely exceeded that on Opus 4.8 (larger output budget × default `effort=high` × multi-step reasoning), surfacing as `"Streaming is required for operations that may take longer than 10 minutes."` `createBetaWithRetry` now always calls `client.beta.messages.create({ ...params, stream: true })` and runs the resulting event stream through `aggregateBetaStream`, which walks `message_start` / `content_block_start` / `content_block_delta` / `content_block_stop` / `message_delta` / `message_stop` events and produces the same `Message` shape the rest of the loop has always consumed. The 10-minute ceiling is replaced with a 30-minute wall-clock budget (`STREAM_TIMEOUT_MS`), enforced via `AbortSignal.any([caller.signal, AbortSignal.timeout(STREAM_TIMEOUT_MS)])` at the call site — the SDK's own `timeout` option only covers up to TTFB so a quiet stall during body consumption needs the composed signal to terminate. This is invisible to the client today — Neo doesn't surface partial tokens via SSE — but it removes the principal blocker for long-running investigations and skill runs.

**Mid-stream errors retry, but you pay twice.** Anthropic's Messages API has no idempotency key. When `aggregateBetaStream` throws partway through a stream (e.g., a 5xx after ~20 minutes of generation), `createBetaWithRetry` catches a retryable status and starts a brand-new stream — a fresh billable model invocation, potentially producing different output. The partial tokens from the failed attempt were already billed by Anthropic, so a 25-min partial + 25-min retry costs ~50 min of Opus output. The retry window expanded from ~10 minutes pre-streaming to up to 30 minutes per attempt × 3 retries; probability of mid-stream double-billing is higher on Opus 4.8 skill workloads. Acceptable tradeoff today (the alternative — refusing to retry — surfaces every transient 5xx as a hard failure) but worth monitoring as Opus 4.8 + skills usage grows.

**Abort semantics.** `signal?.throwIfAborted()` runs at the top of every for-await iteration AND once more AFTER the loop terminates. The post-loop check is load-bearing: the SDK's stream iterator silently returns (does NOT throw) when the underlying fetch is aborted, so without the second check an aborted stream would yield a partial `Message` with `stop_reason: null` and crash the agent loop's stop-reason dispatch. The `isAbortError(err)` helper (used in three places — `createWithRetry`, `createBetaWithRetry`, and the outer `runAgentLoop` catch) matches both runtime `AbortError` and the SDK's own `APIUserAbortError` class so SDK-originated cancellations correctly route to `buildInterruptedResult()` instead of surfacing as opaque errors.

**Malformed tool input.** If the model's `input_json_delta` events concatenate to invalid JSON, `aggregateBetaStream` throws rather than silently defaulting `input` to `{}`. The previous silent-default behaviour could have caused tools to execute with empty arguments (a real data-integrity hazard for executors that scope queries by argument). The throw flows into the retry path; if retries are exhausted, the route surfaces the parse error to the caller.
