...
- **Skill update propagation lag.** The 60 s cache TTL means a freshly-saved skill is invisible on other instances for up to 60 s. The admin who saved it sees their own write immediately (write-through their own instance's cache); other admins see it after refresh. Acceptable trade-off; documented as expected behaviour.
...
- The web app runs on ≥2 App Service instances in a staging slot and passes a load-test plan that exercises:
  ...
  - Skill update via admin API is visible on all instances within 60 s without restart, AND the saving admin sees their write immediately.
...
- **`web/test/skill-store-shared.test.ts`** — dual-source dispatch: `MOCK_MODE` reads the file system; production (Cosmos configured) reads the Cosmos store. Admin write under production mode invalidates the local cache. Cache TTL respected (read-through up to 60 s).
